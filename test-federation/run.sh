...
waitUntilJoined TestFederate 1

cd $root_directory/ExampleGateway/target
cp $root_directory/ExampleGateway/RTI.rid .
cp -r $root_directory/ExampleGateway/conf/ .
xterm -fg cyan -bg black -l -lf $logs_directory/gateway-implementation-${timestamp}.log -T "Gateway Implementation" -geometry 140x40+360+120 -e "java -Dlog4j.configurationFile=conf/log4j2.xml -Djava.net.preferIPv4Stack=true -jar GatewayImplementation-0.0.1-SNAPSHOT.jar conf/GatewayImplementation.json" &
